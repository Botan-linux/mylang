...
class Parser:
    ...
    def parse_method_declaration(self, name: str = None, is_static: bool = False) -> MethodDeclaration:
        ...
        if name is None:
            ...
            # Allow 'new' as method name (constructor)
    ...
    def parse_import_statement(self) -> ImportStatement:
        ...
        if self.check(TokenType.IDENTIFIER) and self.peek().value != 'from':
            ...
            return ImportStatement(module=module, is_all=False, line=import_token.line)
    ...
    def parse_postfix(self) -> ASTNode:
        ...
        while True:
            ...
            # Member access
                ...
                # Allow 'new' as property name (for constructors)
                if self.check(TokenType.NEW):
                    prop_name = 'new'
                    self.advance()
                    prop = StringLiteral(value=prop_name)
                else:
                    prop = self.expect(TokenType.IDENTIFIER, "Expected property name")
                    if isinstance(prop, StringLiteral):
                        pass  # Already a StringLiteral
                    else:
                        prop = StringLiteral(value=prop.value)
...
def create_builtins() -> Dict[str, Any]:
    ...
    def _py_import(name):
        """Import a Python module by name, supporting dotted names like 'os.path'"""
        import importlib
        return importlib.import_module(name)
    
    builtins['__py_import__'] = _py_import
...
class Interpreter:
    ...
    def eval_CallExpression(self, node: CallExpression, env: Environment) -> Any:
        # Special case: ClassName.new() constructor call
        if isinstance(node.callee, MemberExpression):
            obj = self.execute(node.callee.obj, env)
            prop = node.callee.prop.value if not node.callee.computed else self.execute(node.callee.prop, env)
            
            # Check if calling ClassName.new() - constructor pattern
            if isinstance(obj, MyLangClass) and prop == 'new':
                # Create new instance
                instance = MyLangInstance(class_def=obj)
                
                # Copy properties from class
                for name, value in obj.properties.items():
                    instance.properties[name] = value
                
                # Copy inherited properties
                if hasattr(obj, 'parent_class') and obj.parent_class:
                    for name, value in obj.parent_class.properties.items():
                        if name not in instance.properties:
                            instance.properties[name] = value
                
                # Call the 'new' method with instance as self
                args = [self.execute(arg, env) for arg in node.arguments]
                if 'new' in obj.methods:
                    constructor = obj.methods['new']
                    result = self.call_method(constructor, [instance] + args, node.line)
                    # If new returns something, return that; otherwise return instance
    ...
    def eval_MemberExpression(self, node: MemberExpression, env: Environment) -> Any:
        ...
        if isinstance(obj, MyLangClass):
            ...
            # Allow access to regular methods (for constructors like ClassName.new)
            if prop in obj.methods:
                return obj.methods[prop]

        # Built-in types
        if hasattr(obj, prop):
            attr = getattr(obj, prop)
            # If it's a class/type, return it directly for further attribute access
            if isinstance(attr, type):
                return attr
            # If it's callable (function/method), wrap for easy calling
            if callable(attr):
    ...
    def eval_ImportStatement(self, node: ImportStatement, env: Environment) -> Any:
        ...
        # For 'import module_name' (without 'from'), define the module object
        if not node.names and not node.is_all:
            env.define(module_name, module)
        elif node.is_all:
            ...
            # 'from module import name1, name2' - import specific names
    ...
    def load_module(self, name: str) -> Dict[str, Any]:
        ...
        # Try to load from installed packages FIRST (~/.mylang/packages/{name}/{name}.ml)
        package_path = PACKAGES_DIR / name / f"{name}.ml"
        if package_path.exists():
            with open(package_path, 'r', encoding='utf-8') as f:
                source = f.read()
            
            # Create a new interpreter for the module to avoid polluting global env
            ...
            # Return exported items or all defined items
            return module_env.variables

        # Then check standard library modules
        if name in std_modules:
            return std_modules[name]

        # Try to load from file (relative path)
